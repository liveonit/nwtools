...
run_tests:
	docker run --rm -v $(shell pwd)/:/tmp ibarretorey/nwtools:${version} bash -c "cd tmp && sh tests_internal.sh"
	docker run -d --rm --name nwtools_test -v $(shell pwd)/:/tmp -p 880:80 -p 4443:443 ibarretorey/nwtools:${version}
	sleep 5
